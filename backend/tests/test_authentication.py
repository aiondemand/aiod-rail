...
@pytest.mark.asyncio
@patch('app.authentication._verify_token', new_callable=AsyncMock)
async def test_get_current_user_raises_if_token_not_verified(mock_verify_token: AsyncMock):
...
async def test_get_current_user_returns_user_from_api_key(mock_find_one: AsyncMock):
    ...
    user = await get_current_user(required=True, from_token=False, from_api_key=True)(api_key="1234")
...
async def test_get_current_user_raises_if_api_key_invalid(mock_find_one: AsyncMock):
    ...
    with pytest.raises(HTTPException) as exception_info:
        await get_current_user(required=True, from_token=False, from_api_key=True)(api_key="invalid_api_key")
