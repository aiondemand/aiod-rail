...
@Component({
  ...
  templateUrl: '../experiments/pages/templates/templates.html',
  styleUrls: ['../experiments/pages/templates/templates.scss'],
})
export class AllTemplatesPage implements OnInit {
  ...
  private load() {
    ...
    this.api
      .getExperimentTemplates(this.query(), { offset: this.offset(), limit: this.pageSize() })
  ...
  // ---- adapters
  titleOf(t: ExperimentTemplate): string {
    return (t as any).name ?? (t as any).title ?? '';
  }
  descOf(t: ExperimentTemplate): string {
    return (t as any).description ?? (t as any).summary ?? '';
  }
  dateOf(t: ExperimentTemplate): string | number | Date | undefined {
    return (t as any).created_at ?? (t as any).createdAt ?? (t as any).date ?? undefined;
  }
  platformOf(t: ExperimentTemplate): string {
