...
export class BackendApiService {
  ...
  createDataset(
    ...
    if (dataset.platform === 'huggingface') {
      return this.http
        .post<Dataset>(`${environment.BACKEND_API_URL}/assets/datasets`, dataset)
        .pipe(
          switchMap((dataset) =>
            this.uploadDatasetToHuggingFace(dataset, file, hf_username, hf_token)
          )
        );
    }
   ...
   * @returns
   */
  private uploadDatasetToHuggingFace(
    dataset: Dataset,
    file: any,
    hf_username: string,
    hf_token: string
  ...
  private deleteDataset(dataset: Dataset): Observable<boolean> {
    return this.http
      .delete<Dataset>(`${environment.BACKEND_API_URL}/assets/datasets/${dataset.identifier}`)
      .pipe(map((_) => true));
  }
  ...
  getMyDatasets(query: string = '', pageQueries?: PageQueries): Observable<Dataset[]> {
    ...
    return this.http
      .get<Dataset[]>(`${environment.BACKEND_API_URL}/assets/datasets/my${queries}`)
      .pipe(
        // TODO: Implement in MyLibrary and Backend
        // Filter only datasets their name contains th  e query
        map((datasets) =>
          query && datasets ? datasets.filter((dataset) => dataset.name.includes(query)) : datasets
        )
      );
  }
  ...
  getMyModels(query: string = '', pageQueries?: PageQueries): Observable<Model[]> {
    ...
    return this.http.get<Model[]>(`${environment.BACKEND_API_URL}/assets/models/my${queries}`).pipe(
      ...
      map((models) =>
        query && models ? models.filter((model) => model.name.includes(query)) : models
      )
    );
  }
  ...
  getPublication(publicationId: string): Observable<Publication> {
    return this.http.get<Publication>(
      `${environment.BACKEND_API_URL}/assets/publications/${publicationId}`
    );
  }
  ...
  getExperiments(
    ...
    let queries = `?query=${query}&${this._buildPageQueries(
      pageQueries
    )}&${this._buildExperimentFilter(experimentFilter)}`;
  ...
  updateExperiment(id: string, experiment: ExperimentCreate): Observable<Experiment> {
    return this.http.put<Experiment>(
      `${environment.BACKEND_API_URL}/experiments/${id}`,
      experiment
    );
  }
  ...
  archiveExperiment(id: string, archive: boolean): Observable<void> {
    return this.http.patch<void>(
      `${environment.BACKEND_API_URL}/experiments/${id}/archive?archive=${archive}`,
      {}
    );
  }
  ...
  getExperimentTemplates(
    ...
    let queries = `?query=${query}&${this._buildPageQueries(
      pageQueries
    )}&${this._buildExperimentTemplateFilter(experimentTemplateFilter)}`;
    return this.http.get<ExperimentTemplate[]>(
      `${environment.BACKEND_API_URL}/experiment-templates${queries}`
    );
  }
  ...
  getExperimentTemplate(id: string): Observable<ExperimentTemplate> {
    return this.http.get<ExperimentTemplate>(
      `${environment.BACKEND_API_URL}/experiment-templates/${id}`
    );
  }
  ...
  getExperimentTemplatesCount(
    ...
    let queries = `?query=${query}&${this._buildExperimentTemplateFilter(
      experimentTemplateFilter
    )}`;
    return this.http.get<number>(
      `${environment.BACKEND_API_URL}/count/experiment-templates${queries}`
    );
  }

  getExperimentsOfTemplateCount(id: string, only_mine: boolean): Observable<number> {
    return this.http.get<number>(
      `${environment.BACKEND_API_URL}/count/experiment-templates/${id}/experiments?only_mine=${only_mine}`
    );
  }
  ...
  createExperimentTemplate(
    experimentTemplate: ExperimentTemplateCreate
  ): Observable<ExperimentTemplate> {
    return this.http.post<ExperimentTemplate>(
      `${environment.BACKEND_API_URL}/experiment-templates`,
      experimentTemplate
    );
  }
  ...
  updateExperimentTemplate(
    ...
    return this.http.put<ExperimentTemplate>(
      `${environment.BACKEND_API_URL}/experiment-templates/${id}`,
      experimentTemplate
    );
  }
  ...
  archiveExperimentTemplate(id: string, archive: boolean): Observable<void> {
    return this.http.patch<void>(
      `${environment.BACKEND_API_URL}/experiment-templates/${id}/archive?archive=${archive}`,
      {}
    );
  }
  ...
  approveExperimentTemplate(id: string, approve: boolean): Observable<void> {
    return this.http.patch<void>(
      `${environment.BACKEND_API_URL}/experiment-templates/${id}/approve?approve=${approve}`,
      {}
    );
  }
  ...
  getExperimentRuns(experimentId: string, pageQueries?: PageQueries): Observable<ExperimentRun[]> {
    ...
    return this.http.get<ExperimentRun[]>(
      `${environment.BACKEND_API_URL}/experiments/${experimentId}/runs${pageQueriesStr}`
    );
  }

  getExperimentRunsCount(experimentId: string): Observable<number> {
    return this.http.get<number>(
      `${environment.BACKEND_API_URL}/count/experiments/${experimentId}/runs`
    );
  }

  listFilesFromExperimentRun(experimentId: string): Observable<FileDetail[]> {
    return this.http.get<FileDetail[]>(
      `${environment.BACKEND_API_URL}/experiment-runs/${experimentId}/files/list`
    );
  }
  ...
  getExperimentRun(experimentRunId: string): Observable<ExperimentRunDetails> {
    return this.http.get<ExperimentRunDetails>(
      `${environment.BACKEND_API_URL}/experiment-runs/${experimentRunId}`
    );
  }

  stopExperimentRun(experimentRunId: string): Observable<void> {
    return this.http.get<void>(
      `${environment.BACKEND_API_URL}/experiment-runs/${experimentRunId}/stop`
    );
  }
  ...
  getExperimentRunLogs(experimentRunId: string): Observable<string> {
    return this.http.get<string>(
      `${environment.BACKEND_API_URL}/experiment-runs/${experimentRunId}/logs`,
  ...
  executeExperimentRun(experimentId: string): Observable<ExperimentRun> {
    return this.http.get<ExperimentRun>(
      `${environment.BACKEND_API_URL}/experiments/${experimentId}/execute`
    );
  }
