...
import { catchError, combineLatest, of, retry, switchMap, tap } from 'rxjs';
...
export class ExperimentDetailComponent {
  ...
  set id(id: string) {
    ...
    this.backend.getExperiment(id)
      ...
      .subscribe({
        next: ([dataset, model, publications, experimentTemplate, experiment]) => {
