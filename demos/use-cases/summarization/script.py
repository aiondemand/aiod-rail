...
def summarize_data(model, tokenizer, texts):
    """
...
if __name__ == "__main__":
    ...
    # OPTIONAL ENV VARS
    num_datapoints_to_eval = int(os.getenv("NUM_DATAPOINTS_TO_SUMMARIZE", default=-1))
    ...
    for i in tqdm(range(0, num_datapoints_to_eval, batch_size)):
        ...
        summarizations = summarize_data(model, tokenizer, data)

        json_data.extend([{
            "original": orig,
            "summarized": summ
        }] for orig, summ in zip(data, summarizations))

    # store the results
    os.makedirs("./output-temp", exist_ok=True)
    with open("./output-temp/summarizations.json", "w") as f:
