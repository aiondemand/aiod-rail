...
class Authenticator:
    def __init__(self) -> None:
        self._openid_config = requests.get(
            "https://aiod-dev.i3a.es/aiod-auth/realms/aiod/.well-known/openid-configuration"
        ).json()
    ...
    def get_access(self) -> dict:
        ...
        self._response = self._session.fetch_token(
            self._token_uri, authorization_response=authorization_response
        )

        return {
            "Authorization": f"{self._response['token_type']} {self._response['access_token']}"
        }
